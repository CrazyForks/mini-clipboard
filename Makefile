...
dmg-unsigned: app
	...
	hdiutil create -volname "$(PRODUCT_NAME)" -srcfolder "$(STAGE)" -ov -format UDZO "$(DIST)/Mini-Clipboard.dmg"
...
sign-app:
	@if [ -n "$(DEVELOPER_ID)" ]; then \
		...
		--entitlements App/MiniClipboard.entitlements \
...
sign-dmg:
	test -n "$(DEVELOPER_ID)" \
		...
		--sign "$(DEVELOPER_ID)" "$(DIST)/Mini-Clipboard.dmg" || true

notarize:
	@if [ -n "$(NOTARY_PROFILE)" ]; then \
		xcrun notarytool submit "$(DIST)/Mini-Clipboard.dmg" --keychain-profile "$(NOTARY_PROFILE)" --wait; \
	else \
		test -n "$(APPLE_ID)" && test -n "$(TEAM_ID)" && test -n "$(APP_PASSWORD)"; \
		xcrun notarytool submit "$(DIST)/Mini-Clipboard.dmg" --apple-id "$(APPLE_ID)" --team-id "$(TEAM_ID)" --password "$(APP_PASSWORD)" --wait; \
	fi

staple:
	xcrun stapler staple "$(DIST)/Mini-Clipboard.dmg"
